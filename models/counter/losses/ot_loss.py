...
class OT_Loss(Module):
    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        ot_obj_values = torch.zeros((batch_size)).to(self.device)
        wd = [] # wasserstain distance
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                ot_obj_values[idx] = torch.sum(normed_density[idx] * beta.view([1, self.output_size, self.output_size]))
                ...
                loss[idx] = torch.sum(unnormed_density[idx] * im_grad)
                wd.append(torch.sum(dis * P).item())
