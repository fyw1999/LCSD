...
def build_targets(p, targets, model):
    ...
    ai = torch.arange(na, device=targets.device).float().view(na, 1).repeat(1, nt)#anchor's index
    ...
    for i, yolo_layer in enumerate(model.yolo_layers):
        ...
        t = targets * gain #(3,predicted_bbox_num,7)
        ...
        if nt:
            ...
            t = t[j] # all the filtered anchors, There may be duplicates among them, but the anchor's index is different
        else:
            t = targets[0]
